...
このアプリは現在、鉄系(4.6〜14.9)とステンレス系(A2/A4)のボルトのみ収録。
黄銅・アルミ・チタン・樹脂などの材質が必要な場合、メニュー →「材質を追加リクエスト」
から要望を登録できます。

### 入力フォーム
- 材質名(必須): 例「黄銅(C3604)」「アルミ A2017」
- 種類(プルダウン): 銅合金 / 軽合金 / チタン / 特殊ステンレス / 樹脂 等
- 用途・使う場面: 自由記入
- わかる範囲で参考値: Rp/Rm/JIS番号など
- 備考、依頼者名

### 送信方法 (4通り)
1. **メールで送信** — mailto: で本文がプリフィルされ、メールアプリが開く
2. **Googleフォームで送信** — 内容をクリップボードにコピー後、フォームを別タブで開く
3. **クリップボードにコピー** — チャット・メモ等に貼り付けて運用
4. **ローカル保存のみ** — 後でまとめて処理

どの方法でも内容はスマホ内に保存され、「送信したリクエスト」から
再送信・コピー・削除・JSON一括エクスポートが可能。

### 送信先の設定
メニュー →「送信先の設定」で、メール宛先(複数可、カンマ区切り)と
Googleフォーム URL を保存できます。一度設定すれば毎回入力不要。
設定値はスマホ内の localStorage に保存され、端末外に送信されません。
